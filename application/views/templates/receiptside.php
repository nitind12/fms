<div id="sidebar-collapse" class="col-sm-3 col-lg-2 sidebar">
		<div class="profile-sidebar">
			<div class="profile-userpic">
				<img src="http://placehold.it/50/30a5ff/fff" class="img-responsive" alt="user">
			</div>
			<div class="profile-usertitle">
				<div class="profile-usertitle-name">Admin</div>
				<div class="profile-usertitle-status"><span class="indicator label-success"></span>Online</div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="divider"></div>
		<form role="search">
			<div class="form-group">
				<input type="text" class="form-control" placeholder="Search">
			</div>
		</form>
		<ul class="nav menu">
			<li><a href="dashboard"><em class="fa fa-dashboard">&nbsp;</em> Dashboard</a></li>
			<li class="parent "><a data-toggle="collapse" href="#sub-item-1"><em class="fa fa-user-circle-o">&nbsp;</em>
				 Master<span class="caret" data-toggle="collapse" href="#sub-item-1" class="icon pull-right"></span>
				</a>
				<ul class="children collapse" id="sub-item-1">
					<li><a class="" href="master">
						<span class="fa fa-arrow-right">&nbsp;</span> Session
					</a></li>
					<li><a class="" href="staffmaster">
						<span class="fa fa-arrow-right">&nbsp;</span> Class
					</a></li>
					<li><a class="" href="master">
						<span class="fa fa-arrow-right">&nbsp;</span> Class in session
					</a></li>
					<li><a class="" href="staffmaster">
						<span class="fa fa-arrow-right">&nbsp;</span> Static heads
					</a></li>
					<li><a class="" href="master">
						<span class="fa fa-arrow-right">&nbsp;</span> Flexible heads
					</a></li>
					<li><a class="" href="staffmaster">
						<span class="fa fa-arrow-right">&nbsp;</span> Static in class
					</a></li>
					<li><a class="" href="master">
						<span class="fa fa-arrow-right">&nbsp;</span> Flexihead to students
					</a></li>
					<li><a class="" href="staffmaster">
						<span class="fa fa-arrow-right">&nbsp;</span> Drop student
					</a></li>
					
					
				</ul>
			</li>
			<li><a href="registration"><em class="fa fa-graduation-cap">&nbsp;</em> Registration</a></li>
			<li class="active"><a href="fees"><em class="fa fa-credit-card">&nbsp;</em> Invoice/Receipt</a></li>
			<li class="parent "><a data-toggle="collapse" href="#sub-item-2"><em class="fa fa-book">&nbsp;</em>
				 Reports<span class="caret" data-toggle="collapse" href="#sub-item-2" class="icon pull-right"></span>
				</a>
				<ul class="children collapse" id="sub-item-2">
					<li><a class="" href="dreports">
						<span class="fa fa-arrow-right">&nbsp;</span> Daily Reports
					</a></li>
					<li><a class="" href="breports">
						<span class="fa fa-arrow-right">&nbsp;</span> Balance Reports
					</a></li>
					<li><a class="" href="reports">
						<span class="fa fa-arrow-right">&nbsp;</span> Delete Fees Backup
					</a></li>
				</ul>
			</li>
			<li><a href="login"><em class="fa fa-power-off">&nbsp;</em> Logout</a></li>
		</ul>
	</div>